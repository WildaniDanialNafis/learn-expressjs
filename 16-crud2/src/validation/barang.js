...
const sanitization = (data) => {
    return {
        ...
        expired_date: validator.escape(validator.trim(data.kadaluarsa)),
    }
}
...
const barangValid = (dt) => {
    ...
    if (validator.isEmpty(data.jumlah)) {
        message.push("Jumlah barang tidak boleh kosong");
    }
    if (validator.isEmpty(data.harga_satuan)) {
        message.push("Harga satuan barang tidak boleh kosong");
    }
